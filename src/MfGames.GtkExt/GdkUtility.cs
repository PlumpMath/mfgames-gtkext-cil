...
namespace MfGames.GtkExt
{
	...
	public static class GdkUtility
	{
		...
		{
			return (int) (keyMappings.ContainsKey(key)
				? keyMappings[key]
				: key);
		}
		...
		{
			var m = (uint) (((modifier & ModifierType.ControlMask) != 0)
				? 1
				: 0);

			m = (m << 1) | (uint) (((modifier & ModifierType.ShiftMask) != 0)
				? 1
				: 0);
			m = (m << 1) | (uint) (((modifier & ModifierType.MetaMask) != 0)
				? 1
				: 0);
			m = (m << 1) | (uint) (((modifier & ModifierType.Mod1Mask) != 0)
				? 1
				: 0);
			m = (m << 1) | (uint) (((modifier & ModifierType.SuperMask) != 0)
				? 1
				: 0);
